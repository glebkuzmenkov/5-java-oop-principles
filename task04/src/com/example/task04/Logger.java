...
public class Logger {
    ...
    public Logger(String name,  HandleEnum handleType) {
        this.name = name;
        messageHandler = handleType.getHandle();
    }
    ...
    public void log(Level level, String message){
        ...
        if(level.compareTo(this.level) >= 0){
            messageHandler.Handle(formatMessage(level, message));
        }
    }
    public void log(Level level, String format, Object... args){
        if(level.compareTo(this.level) >= 0){
            messageHandler.Handle(formatMessage(level, String.format(format, args)));
        }
    }
